...
def init_generator_weights(net, init_type='normal', init_gain=0.02, encoder_only=False):
    """Initialize network weights.
    ...
    def init_func(m):  # define the initialization function
        classname = m.__class__.__name__
        if hasattr(m, 'weight') and (classname.find('ConvTranspose2d') != -1 or classname.find('Linear') != -1):
            if init_type == 'normal':
                init.normal_(m.weight.data, 0.0, init_gain)
            elif init_type == 'xavier':
                init.xavier_normal_(m.weight.data, gain=init_gain)
            ...
            if hasattr(m, 'bias') and m.bias is not None:
                init.constant_(m.bias.data, 0.0)
        elif hasattr(m, 'weight') and (classname.find('Conv2d') != -1) and (not encoder_only):
            if init_type == 'normal':
                init.normal_(m.weight.data, 0.0, init_gain)
            elif init_type == 'xavier':
                init.xavier_normal_(m.weight.data, gain=init_gain)
            ...
            if hasattr(m, 'bias') and m.bias is not None:
                init.constant_(m.bias.data, 0.0)
        elif classname.find('BatchNorm2d') != -1 and (not encoder_only):  # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
            init.normal_(m.weight.data, 1.0, init_gain)
            init.constant_(m.bias.data, 0.0)
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = BigStrideUnetGenerator(input_nc, output_nc, 5, ngf, norm_layer=norm_layer, use_dropout=use_dropout, stride=4)
    elif netG == 'unet_128':
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[], mask=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, mask=mask)
    elif netD == 'n_layers':  # more options
        net = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, mask=mask)
...
class BigStrideUnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, stride=2):
        """Construct a Unet generator
        Parameters:
            input_nc (int)  -- the number of channels in input images
        ...
        It is a recursive process.
        """
        super(BigStrideUnetGenerator, self).__init__()
        unet_block = BigStrideUnetSkipConnectionBlock(ngf * 2, ngf * 2, input_nc=None, submodule=None, norm_layer=norm_layer, stride=stride, innermost=True)
        unet_block = BigStrideUnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer, stride=stride)
        self.model = BigStrideUnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer, stride=stride)  # add the outermost layer

    def forward(self, input):
        """Standard forward"""
        return self.model(input)

class BigStrideUnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False, stride=4):
        ...
        """
        super(BigStrideUnetSkipConnectionBlock, self).__init__()
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, mask=None, norm_layer=nn.BatchNorm2d):
...
class TaskNetwork(nn.Module):
    ...
    def __init__(self, input_nc = 1, ndf=32, mask=None, norm_layer=nn.InstanceNorm2d):
...
def define_T(mask=None, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = TaskNetwork(mask=mask)
